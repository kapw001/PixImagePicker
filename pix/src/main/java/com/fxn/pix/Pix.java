...
public class Pix extends AppCompatActivity implements View.OnTouchListener {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    initialize();
    dir = new File(Environment.getExternalStorageDirectory(), options.getPath());
    photo1 = new File(dir,
        "IMG_"
  ...
  private void initialize() {
    ...
    int width = size.x;
    int height = size.y;
    cameraView.setPreviewResolution(new Resolution(width,height));*/
      ...
      public void onClick(View view) {
        ...
        fotoapparat.takePicture().toBitmap().transform(new Function1<BitmapPhoto, Bitmap>() {
          ...
          public Bitmap invoke(BitmapPhoto bitmapPhoto) {
            rotationDegrees = 0;
            Log.e("click ", "time ");
            Bitmap b = Utility.rotate(bitmapPhoto.bitmap, bitmapPhoto.rotationDegrees);
            Log.e("click ", "time 0");
            return b;
          }
          ...
          public Unit invoke(final Bitmap bitmap) {
            if (bitmap != null) {
              Utility.vibe(Pix.this, 50);
              // bitmap =  Utility.rotate(bitmap, -rotationDegrees);
              Log.e("click ", "time 1");
              oj.cancel();
              new Thread(new Runnable() {
                @Override public void run() {
                  Utility.writeImage(photo1, bitmap, options.getPath(), options.getImageQuality(),
                      options.getWidth(), options.getHeight());
                }
              }).run();
              Log.e("click ", "time 2");
              // selectionList.clear();
              Img img = new Img("", "", photo1.getAbsolutePath(), "");
              img.setOrientation(rotationDegrees);
              selectionList.add(img);
              if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
                final Intent scanIntent = new Intent(Intent.ACTION_MEDIA_SCANNER_SCAN_FILE);
                final Uri contentUri = Uri.fromFile(photo1);
                scanIntent.setData(contentUri);
                sendBroadcast(scanIntent);
              } else {
                sendBroadcast(
                    new Intent(Intent.ACTION_MEDIA_MOUNTED, Uri.parse(photo1.getAbsolutePath())));
              }
              returnObjects();
    ...
    front.setOnClickListener(new View.OnClickListener() {
      ...
      public void onClick(View view) {
        ...
        oa1.addListener(new AnimatorListenerAdapter() {
          ...
          public void onAnimationEnd(Animator animation) {
            super.onAnimationEnd(animation);
            front.setImageResource(R.drawable.ic_switch_camera_black_24dp);
            oa2.start();
          }
        });
        oa1.start();
